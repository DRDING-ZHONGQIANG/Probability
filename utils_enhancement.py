...
def lossMask_enhanced(mix, sph, shape, device):
    TONAL = 1;     NON_TONAL = 2;     IRRELEVANT = 3;  FFT_SIZE = 320
    loss_mask_feat = torch.zeros(shape, dtype=torch.float32, device=device)
    loss_mask_lbl = torch.zeros(shape, dtype=torch.float32, device=device)
    batch = shape[0]; channel = shape[1]; win = shape[2]

    for i in range(batch):
        for j in range(channel):
            feat_data = mix[i,i*160:i*160+160].numpy(); lbl_data = sph[i, i*160:i*160+160].numpy()
            ...
            for k in range(win-1):
                if feat_flags[k]== TONAL:
                    loss_mask_feat[i,j,k] = 1
                elif feat_flags[k]== NON_TONAL:
                    loss_mask_feat[i, j, k] = 0.6
                elif feat_flags[k]== IRRELEVANT :
                    loss_mask_feat[i, j, k] = 0.3
                if lbl_flags[k] == TONAL:
                    loss_mask_lbl[i, j, k] = 1
                elif lbl_flags[k] == NON_TONAL:
                    loss_mask_lbl[i, j, k] = 0.6
                elif lbl_flags[k] == IRRELEVANT:
                    loss_mask_lbl[i, j, k] = 0.3
    return loss_mask_feat, loss_mask_lbl

...
def main():
    with open('C:/PycharmProjects/Cruse5/filelists/tr_list.txt', 'r') as f:
        tr_list = [line.strip() for line in f.readlines()]
    for i, in_file in enumerate(tr_list):
        reader = h5py.File(in_file, 'r')
        mix    = np.float32(reader['mix'][:])
        sph    = np.float32(reader['sph'][:])
        reader.close()
        win = 320; hop = 160
        channel = int(len(mix)/hop)+1
        loss_mask_feat, loss_mask_lbl, feat, lbl = lossMask(mix, sph,channel,win,hop)
        loss_mask_feat=loss_mask_feat.flatten(); loss_mask_lbl=loss_mask_lbl.flatten();feat= feat.flatten(); lbl = lbl.flatten()
        writer = h5py.File(in_file+"_mask", 'w')
        minshape = min(loss_mask_feat.shape, loss_mask_lbl.shape)
        writer.create_dataset('loss_mask_feat', data=loss_mask_feat, dtype=loss_mask_feat.dtype, shape=minshape,chunks=True)
        writer.create_dataset('loss_mask_lbl',  data=loss_mask_lbl, dtype=loss_mask_lbl.dtype, shape=minshape,chunks=True)
        writer.create_dataset('feat_psd', data=feat, dtype=feat.dtype, shape=minshape,chunks=True)
        writer.create_dataset('lbl_psd', data=lbl, dtype=lbl.dtype, shape=minshape,chunks=True)
        writer.close()
    return

def lossMask(mix, sph, channel, win, hop):
    TONAL = 1;     NON_TONAL = 2;     IRRELEVANT = 3;  FFT_SIZE = 320
    loss_mask_feat = np.zeros((channel,hop+1), dtype=float)
    loss_mask_lbl  = np.zeros((channel,hop+1), dtype=float)
    feat = np.zeros((channel, hop+1), dtype=float)
    lbl  = np.zeros((channel, hop+1), dtype=float)
    for j in range(channel-1):
            print(j)
            feat_data = mix[j*hop:j*hop+win]; feat_data = feat_data / np.max(np.abs(feat_data))
            lbl_data  = sph[j*hop:j*hop+win];  lbl_data = lbl_data / np.max(np.abs(lbl_data))
            feat_P = np.maximum(20 * np.ma.log10(np.abs(scipy.fftpack.fft(feat_data)[:(int(FFT_SIZE / 2) + 1)]) / FFT_SIZE), -200)
            lbl_P  = np.maximum(20 * np.ma.log10(np.abs(scipy.fftpack.fft(lbl_data)[:(int(FFT_SIZE / 2) + 1)]) / FFT_SIZE), -200)
            feat_P = feat_P.filled(); lbl_P  = lbl_P.filled()
            Delta  = 96 - np.max(feat_P);  feat_P = feat_P + Delta
            Delta  = 96 - np.max(lbl_P);   lbl_P  = lbl_P + Delta
            feat_flags  = find_tonal_set(feat_P, 320);  feat_flags  = find_non_tonal_set(feat_flags, feat_P, 320)
            lbl_flags   = find_tonal_set(lbl_P, 320);   lbl_flags   = find_non_tonal_set(lbl_flags, lbl_P, 320)
            for k in range(hop):
                if feat_flags[k]== TONAL:
                    loss_mask_feat[j,k] = 1
                elif feat_flags[k]== NON_TONAL:
                    loss_mask_feat[j, k] = 0.6
                elif feat_flags[k]== IRRELEVANT :
                    loss_mask_feat[j, k] = 0.3
                if lbl_flags[k] == TONAL:
                    loss_mask_lbl[j, k] = 1
                elif lbl_flags[k] == NON_TONAL:
                    loss_mask_lbl[j, k] = 0.6
                elif lbl_flags[k] == IRRELEVANT:
                    loss_mask_lbl[j, k] = 0.3
                feat[j,k] =  feat_P[k]
                lbl[j,k]  =  lbl_P[k]
    return loss_mask_feat, loss_mask_lbl, feat, lbl

def find_tonal_set(psd, FFT_SIZE):
    """
    ...
    """
    NOT_EXAMINED = 0;    TONAL = 1;     NON_TONAL = 2;   IRRELEVANT = 3; INDEX = 0
    Flags = np.zeros(int(FFT_SIZE / 2), dtype=int) + int(NOT_EXAMINED)
    ...
    if local_max_list.shape[0] > 0:
        for i in range(len(local_max_list[:, 0])):
            ...
            if is_tonal:
                the_psd_val = 10 * np.ma.log10(10 ** (psd[k - 1] / 10) + 10 ** (np.float32(psd[k]) / 10) + 10 ** (np.float32(psd[k + 1]) / 10))
                the_pair = [k, the_psd_val]
                Tonal_list = np.concatenate((Tonal_list, np.reshape(np.array(the_pair), (-1, 2))))
                Flags[k] = TONAL
                for j in np.concatenate((J, [-1, 1])):
                    Flags[k + j] = np.int(IRRELEVANT)
    return Flags


def find_non_tonal_set(Flags, psd, FFT_SIZE):
    """
    ...
    Reference: ISO/IEC 11172-3:1993, Information technology Coding of moving pictures
                ...
                Part 3: Audio, with the permission of ISO.
    """
    NOT_EXAMINED = 0;    TONAL = 1;    NON_TONAL = 2;    IRRELEVANT = 3;
    MIN_POWER = -200;    INDEX = 0;    BARK = 1;     SPL = 1;     ATH = 2
    CB, C, TH, LTq, Map = acoustic_parameter_setup(16000)
        ...
        for k in range(int(TH[CB[i], INDEX]), int(TH[CB[i + 1], INDEX])):  # In each critical band
            ...
            if k < int(FFT_SIZE/2):
                if Flags[k] == NOT_EXAMINED:
                    power = 10 * np.ma.log10(10 ** (power / 10) + 10 ** (psd[k]/ 10))
                    weight = weight + 10 ** (psd[k] / 10) * (TH[Map[k], BARK] - TH[CB[i], BARK])
                    Flags[k] = IRRELEVANT
        if power <= MIN_POWER:
            index = int(int(TH[CB[i], INDEX]))
        else:
            index = int(int(TH[CB[i], INDEX]) + np.round(
                weight / 10 ** (power / 10) * (int(TH[CB[i + 1], INDEX]) - int(TH[CB[i], INDEX]))))
        if index < 0:
            index = 0
        if index > len(Flags) - 1:
            index = int(len(Flags) - 1)
